...
/*css for TOP-COMPLAINT page*/
.table_content1 td, .table_content1 th {
    border: 1px solid #ddd;
    padding: 8px;
}

.table_content1 tr:hover {background-color: #ddd;}
</style>
</head>
<body>
				...
				<table class="table_content1">
					...
					<tbody>
				       ...
				       $sql_reply_count = mysqli_query($conn, "SELECT COUNT(rep_text) FROM reply WHERE post_id=$post_id");
						...
						<tr>
							<td><?php echo $sl; ?></td>
							<td><a href="#"  onclick="MyWindow=window.open('single_post_view.php?post_id=<?php echo $row['post_id']; ?>', 'MyWindow', 'width=800, height=700'); return false;"><?php echo $row['post_title']; ?></a></td>
			...
			<section id="content3" style="margin-right: 2%;">
				...
				<table class="table_content1">
					...
					<tbody>
						...
						// $current_end_date=date('Y-m-t');
						// $Privious_last_date=date('Y-m-t', strtotime("last month"));
							$sql=mysqli_query($conn, "SELECT * FROM complaint ORDER BY post_id DESC");
								$sl=1;
							while ($row=mysqli_fetch_array($sql)) {
							    $post_id= $row['post_id'];
